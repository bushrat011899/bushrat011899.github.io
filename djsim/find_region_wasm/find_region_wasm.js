...
export class RegionJob {
    ...
    */
    get results() {
        try {
            ...
            wasm.regionjob_results(retptr, this.ptr);
    ...
    */
    get done() {
        const ret = wasm.regionjob_done(this.ptr);
        return ret !== 0;
    }
    /**
    * @returns {any}
    */
    next_point() {
        const ret = wasm.regionjob_next_point(this.ptr);
    ...
    */
    run(f) {
        try {
            wasm.regionjob_run(this.ptr, addBorrowedObject(f));
        } finally {
...
async function init(input) {
    if (typeof input === 'undefined') {
        input = new URL('find_region_wasm_bg.wasm', import.meta.url);
    }
    const imports = getImports();
